...
LIBAISUTIL_CONFIG_VARIABLE_HANDLER(LanguageConfig::varHandler)
{
   ...
   debug("LanguageConfig::varHandler() - Attemping to open '" + defName + 
	 '\'');
#endif
   
   // Open the file
   std::ifstream file(defName.c_str());
...
#ifdef KINE_DEBUG_PSYCHO
   debug("LanguageConfig::varHandler() - Beginning read loop");
#endif
   ...
   for (;;) {
...
#ifdef KINE_DEBUG_PSYCHO
      debug("LanguageConfig::varHandler() - Line " + String::convert(lineNum) +
	    " data: '" + line + '\'');
#endif
	 ...
	 Languages::tagDictionary_type::const_iterator it =
	   languages().tagDictionary.find(tag);
	 
	 // Did we find it?
	 if (it == languages().tagDictionary.end()) {
	    // We did not find it - make a new tag..
	    tagID = ++languages().highestTagID;
	    
#ifdef KINE_DEBUG_PSYCHO
	 debug("LanguageConfig::varHandler() - Line " +
	       String::convert(lineNum) + " tag: '" + tag + "' - TID #" +
	       String::convert(tagID) + " (new)");
#endif

	    // .. and insert its TUID into the tag dictionary
	    languages().tagDictionary.
	      insert(Languages::tagDictionary_type::value_type(tag, tagID));
	 } else {
